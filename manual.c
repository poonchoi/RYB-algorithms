...
float calculate_stress (float volume, float bpm) {
	float a = 0; // lowest volume for baby
	float stress = 0;
...
void main (void) {
	...
	while (1) {
		...
		float stress = calculate_stress(input.x, input.y); // calculate stress %
		printf("stress = %.0f\n", stress);
		
		pos = calm(pos, stress); // find new position in matrix
		printf("pos = ");
		print_vec(pos);
